...
DialogForNewGame::DialogForNewGame(wxWindow* parent,struct USERS* users) : wxDialog(parent, 1, language->getValue("dialogForInputNewNameTitle"), wxDefaultPosition, wxDefaultSize)
{
	this->Show(true);
...
void DialogForNewGame::OnButtonOk(wxCommandEvent& event)
{
	...
	{
		this->textError->SetValue(language->getValue("dialogForInputNewNameTextError"));
	}
}
