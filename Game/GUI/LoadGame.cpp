...
LoadGame::LoadGame(wxWindow* parent,struct USERS* users,int wight, int height) : wxDialog(parent, 1, language->getValue("dialogForLoadGameTitle"), wxDefaultPosition, wxDefaultSize)
{
	this->SetSize(height/6,2*wight/3);
	this->users = users;
	text = new wxStaticText(this,wxID_ANY,language->getValue("dialogForLoadGameText"),wxPoint(height/36,wight/24),wxSize(height/12,wight/24));
	...
	load = new wxButton(this, 3, language->getValue("dialogForLoadGameLoad") ,wxPoint(height/36, 7*wight/12),wxSize(height/24, wight/24));
	cancel = new wxButton(this, 4, language->getValue("CancelButton") ,wxPoint(height/36+height/12, 7*wight/12),wxSize(height/24, wight/24));
}
